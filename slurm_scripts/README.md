##	Training LUMOS on a Slurm Cluster
### Starting a training
```bash
$ cd $LUMOS_ROOT/slurm_scripts
$ python slurm_training.py --venv lumos_venv --train_file "../scripts/train_wm.py"
```
This assumes that `--venv lumos_venv` specifies a conda environment.
...
In order to ensure reproducibility, the current state of the lumos repository is copied to the log directory at *submit time* and is
