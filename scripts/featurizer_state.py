...
def featurizer(cfg: DictConfig) -> None:
    ...
    if chk is None:
        ...
        if cfg.world_model.name == "dreamer_v2_state":
            ...
            world_model = DreamerV2_State.load_from_checkpoint(chk.as_posix()).to(cfg.device)
            world_model.eval()
        else:
    ...
    cfg.datamodule.datasets.state_dataset.num_workers = 2
...
def extract_features(world_model, data_loader, dataset, cfg):
    ...
    for epoch in range(EPOCH):
        ...
        for i, batch in enumerate(loader):
            ...
            zfeats[idxs] = obs_to_zero_feature(world_model, batch["state_obs"]).cpu().numpy().squeeze(0)
...
def obs_to_zero_feature(wm, state_obs):
    bz = state_obs.size(1)
