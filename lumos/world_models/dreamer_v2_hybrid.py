...
class DreamerV2_Hybrid(WorldModel):
    ...
    def forward(
        ...
        dcd_img_s, dcd_img_g, dcd_state_obs = self.decoder(features)
    ...
    def pred_img(self, prior: Tensor, features: Tensor) -> Tensor:
        with torch.no_grad():
            ...
            dcd_img_s, dcd_img_g, dcd_state_obs = self.decoder(features_prior)
            return dcd_img_s, dcd_img_g, dcd_state_obs
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Dict[str, Union[Tensor, Any]]:
        ...
        if self.global_step % self.trainer.log_every_n_steps == 0:
            pred_img_s, pred_img_g, pred_state_obs = self.pred_img(*samples)
            self.log_images(
            ...
            self.spawn_env()
            state_img_s, state_img_g = self.render_environment(batch["hybrid"]["state_obs"][-1, 0])
            dcd_state_img_s, dcd_state_img_g = self.render_environment(outs["dcd_state_obs"][-1, 0])
            pre_state_img_s, pre_state_img_g = self.render_environment(pred_state_obs[-1, 0])
    ...
    def validation_step(self, batch: Dict[str, Tensor], batch_idx: int) -> Dict[str, Union[Tensor, Any]]:
        ...
        self.val_state_img_s, self.val_state_img_g = self.render_environment(batch["hybrid"]["state_obs"][-1, 0])
        self.val_dcd_state_img_s, self.val_dcd_state_img_g = self.render_environment(outs["dcd_state_obs"][-1, 0])
        return losses["loss_total"]
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        pred_img_s, pred_img_g, pred_state_obs = self.pred_img(*self.val_samples)
        self.log_images(
            self.val_gt_img_s,
            self.val_gt_img_g,
            self.val_dcd_img_s,
        ...
        # Log images by rendering the environment from the state
        pred_state_img_s, pred_state_img_g = self.render_environment(pred_state_obs[-1, 0])
        self.log_images_state(
    ...
    def loss(self, batch: Dict[str, Tensor], outs: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        loss_state_reconstr = 0.5 * torch.square(outs["dcd_state_obs"] - batch["hybrid"]["state_obs"]).sum(
            dim=[-1, -2, -3]
        )
