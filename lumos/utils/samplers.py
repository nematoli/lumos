...
class ActionChunkEquiSampler(Sampler):
    """Equidistant batch sampler.

    Yields n (where n==batch_size) equidistant indices, steps through the dataset by adding the sequence length to each index and yielding the new set of indices.
    """

    def __init__(self, data_size, seq_len, batch_size, action_chunk, init_idx=None):
        self.data_size = data_size
        self.seq_len = seq_len
        self.batch_size = batch_size
        self.init_idx = init_idx
    ...
    def __iter__(self):
        ...
        for j in range(self.batch_size):
            for i in range(self.n_steps):
                iters[i].append(init_idx + idx)
                idx += self.action_chunk
                if init_idx + idx >= self.data_size:
                    offset += 1
                    idx = offset % self.action_chunk
